  ...
  try {
    ...
    if (!type || !category || !amount || !description || !reference_date) {
      return NextResponse.json(
        {
          error:
            "Campos obrigatórios: type, category, amount, description, reference_date",
        },
        { status: 400 },
      );
    }
    ...
    // Usa transação do Prisma para garantir atomicidade: cria o lançamento e, se houver
    // referência de mensalidade, marca-a como paga em uma única operação
    const novaTransacao = await prisma.$transaction(async (tx) => {
      const transacaoCriada = await tx.financial_transactions.create({
        data: {
          type,
          category,
          amount,
      ...
      // Marca a mensalidade como paga se o lançamento estiver vinculado a uma
      if (monthly_fee_id) {
        await tx.monthly_fees.update({
          where: { id: monthly_fee_id },
          data: {
            status: "paid",
            paid_at: new Date(reference_date),
