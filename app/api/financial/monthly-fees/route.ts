...
export async function GET(request: NextRequest) {
    try {
        ...
        // Jogador comum só pode ver as próprias mensalidades
        const filtroUsuario = ehGestor ? userId : user.id

        const mensalidades = await prisma.monthly_fees.findMany({
            where: {
                ...(filtroUsuario && { user_id: filtroUsuario }),
                ...(mes && { month: mes }),
                ...(ano && { year: ano }),
...
export async function POST(request: NextRequest) {
    try {
        ...
        const valorMensalidade = configuracoes?.monthly_fee ?? 80.00
