...
export async function PATCH(
  ...
  try {
    ...
    const jogadorExistente = await prisma.users.findUnique({
      where: { id },
      select: { id: true },
    });
    if (!jogadorExistente) {
    ...
    if (birth_date !== undefined)
      dadosParaAtualizar.birth_date = birth_date ? new Date(birth_date) : null;

    // Campos restritos a admin/co-admin
    if (temPrivilegioDeAdmin) {
      if (is_goalkeeper !== undefined)
        dadosParaAtualizar.is_goalkeeper = is_goalkeeper;
