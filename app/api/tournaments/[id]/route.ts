...
function labelDaRodadaBracket(indiceRodada: number, totalRodadas: number): string {
    ...
    switch (distanciaDoFinal) {
        ...
        default: return `Rodada ${indiceRodada + 1}`
    }
}
...
function gerarRodadasMataMata(times: { id: string; team_name: string }[]): {
    rodada: number
    jogos: { casaId: string | null; casaNome: string; visitaId: string | null; visitaNome: string }[]
}[] {
    if (times.length < 2) return []

    const n = times.length
    const metade = Math.floor(n / 2)
    const primeiraRodada: { casaId: string | null; casaNome: string; visitaId: string | null; visitaNome: string }[] =
            ...
            for (const [ir, jogos] of rodadas.entries()) {
                for (const [ij, [casaId, visitaId]] of jogos.entries()) {
                    ...
                    await tx.match_teams.createMany({
                        data: [
                            { match_id: partida.id, team_name: timePorId.get(casaId) ?? casaId, team_index: 0, tournament_team_id: casaId },
                            { match_id: partida.id, team_name: timePorId.get(visitaId) ?? visitaId, team_index: 1, tournament_team_id: visitaId },
                        ],
                    })
                }
            }
            ...
            if (leagueLegs === 2) {
                ...
                for (const [ir, jogos] of rodadas.entries()) {
                    for (const [ij, [casaId, visitaId]] of jogos.entries()) {
                        ...
                        await tx.match_teams.createMany({
                            data: [
                                { match_id: partida.id, team_name: timePorId.get(visitaId) ?? visitaId, team_index: 0, tournament_team_id: visitaId },
                                { match_id: partida.id, team_name: timePorId.get(casaId) ?? casaId, team_index: 1, tournament_team_id: casaId },
                            ],
                        })
                    }
                }
...
export async function GET(
    ...
    try {
                ...
                tournament_teams: {
                    ...
                    include: {
                        tournament_team_players: {
                            include: {
                                users: { select: { id: true, name: true, nickname: true, photo_url: true, is_goalkeeper: true, position: true, player_ratings: { select: { overall: true } } } },
                                guest_players: { select: { id: true, name: true, is_goalkeeper: true, position: true, overall: true } },
                            },
                        },
                    },
                },
        ...
        const matchIds = campeonato.tournament_stages.flatMap(s => s.matches.map(m => m.id))

        // Pré-inicializa placar zerado para todas as partidas (inclui pênaltis já salvos no banco)
        const matchScores: Record<string, { homeScore: number; awayScore: number; penaltyHomeScore: number | null; penaltyAwayScore: number | null }> = {}
        ...
        let artilharia: { id: string; nome: string; gols: number; ehGuest: boolean }[] = []
        let goleiros: { id: string; nome: string; golsSofridos: number; partidas: number; ehGuest: boolean }[] = []
        ...
        if (matchIds.length > 0) {
            ...
            // Artilharia
            const byPlayer = new Map<string, { nome: string; gols: number; ehGuest: boolean }>()
            for (const goal of goalsRaw) {
                if (goal.scorer_user_id && goal.users) {
                    const key = `user:${goal.scorer_user_id}`
                    const entry = byPlayer.get(key) ?? { nome: goal.users.nickname ?? goal.users.name, gols: 0, ehGuest: false }
                    entry.gols++
                    byPlayer.set(key, entry)
                } else if (goal.scorer_guest_id && goal.guest_players) {
                    const key = `guest:${goal.scorer_guest_id}`
                    const entry = byPlayer.get(key) ?? { nome: goal.guest_players.name, gols: 0, ehGuest: true }
                    entry.gols++
                    byPlayer.set(key, entry)
                }
            }
            ...
            const teamIndexByMatch = new Map<string, { homeId: string | undefined; awayId: string | undefined }>()
            for (const stage of campeonato.tournament_stages) {
                for (const match of stage.matches) {
            ...
            const byGk = new Map<string, { nome: string; golsSofridos: number; partidasSet: Set<string>; ehGuest: boolean }>()
            for (const mp of matchPlayersRaw) {
                if (!mp.is_goalkeeper) continue
                if (mp.user_id && mp.users) {
                    const key = `user:${mp.user_id}`
                    const entry = byGk.get(key) ?? { nome: mp.users.nickname ?? mp.users.name, golsSofridos: 0, partidasSet: new Set<string>(), ehGuest: false }
                    entry.partidasSet.add(mp.match_id)
                    byGk.set(key, entry)
                } else if (mp.guest_player_id && mp.guest_players) {
                    const key = `guest:${mp.guest_player_id}`
                    const entry = byGk.get(key) ?? { nome: mp.guest_players.name, golsSofridos: 0, partidasSet: new Set<string>(), ehGuest: true }
                    entry.partidasSet.add(mp.match_id)
                    byGk.set(key, entry)
                }
            }
            for (const gc of gcRaw) {
                if (gc.conceder_user_id && gc.users) {
                    ...
                    const entry = byGk.get(key) ?? { nome: gc.users.nickname ?? gc.users.name, golsSofridos: 0, partidasSet: new Set<string>(), ehGuest: false }
                    ...
                    const entry = byGk.get(key) ?? { nome: gc.guest_players.name, golsSofridos: 0, partidasSet: new Set<string>(), ehGuest: true }
                    entry.golsSofridos += gc.amount
                    entry.partidasSet.add(gc.match_id)
                    byGk.set(key, entry)
                }
            ...
            goleiros = Array.from(byGk.entries())
                .map(([id, v]) => ({ id, nome: v.nome, golsSofridos: v.golsSofridos, partidas: v.partidasSet.size, ehGuest: v.ehGuest }))
                .sort((a, b) => a.golsSofridos - b.golsSofridos)
                .slice(0, 10)
        }

        return NextResponse.json({ ...campeonato, artilharia, goleiros, match_scores: matchScores })
    } catch (error) {
...
export async function PATCH(
    ...
    try {
        ...
        const { name, status, location, start_date, end_date, description, num_teams, squad_size, settings, champion_team_id } = body
        ...
        if (champion_team_id !== undefined) dadosParaAtualizar.champion_team_id = champion_team_id
        ...
        if (status === 'active') {
            ...
            if (stagesExistentes === 0) {
                ...
                if (campeonatoParaGerar && campeonatoParaGerar.tournament_teams.length >= 2) {
