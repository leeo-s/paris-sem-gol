...
export async function POST(
    ...
    try {
        ...
        const { voted_user_id } = body

        if (!voted_user_id) {
            return NextResponse.json({ error: 'voted_user_id é obrigatório' }, { status: 400 })
        }
        ...
        // Verifica se a votação ainda está no prazo
        ...
        // Verifica se o votado estava presente na partida
        ...
        const novoVoto = await prisma.$transaction(async (tx) => {
            ...
            // Atualiza o total de votos na sessão para checar se todos já votaram
            ...
            await tx.mvp_voting_sessions.update({
                ...
                data: {
                    ...
                    // Fecha automaticamente quando todos os elegíveis votarem
