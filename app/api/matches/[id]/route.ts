...
export async function PATCH(
  ...
  try {
    ...
    const { status, location, match_date, team_assignments, title } = body;
    ...
    if (status !== undefined)
      dadosParaAtualizar.status = status as match_status;
    if (location !== undefined) dadosParaAtualizar.location = location;
    if (match_date !== undefined)
      dadosParaAtualizar.match_date = new Date(match_date);
    ...
    const partidaAtualizada = await prisma.$transaction(async (tx) => {
      ...
      if (
        status === "started" &&
        Array.isArray(team_assignments) &&
        team_assignments.length > 0
      ) {
        for (const atribuicao of team_assignments as {
  ...
  try {
    ...
    if (partidaExistente.status !== "scheduled") {
      return NextResponse.json(
        {
          error:
            "Não é possível excluir uma partida que já foi iniciada ou encerrada",
        },
        { status: 422 },
      );
    }
