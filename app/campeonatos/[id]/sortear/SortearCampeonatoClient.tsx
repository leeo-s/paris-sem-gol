    ...
    <div className="bg-card rounded-xl ring-1 ring-foreground/10 overflow-hidden">
      ...
      <div className="divide-y divide-border">
        {time.jogadores.map((jogador) => (
          <CardJogador key={jogador.registrationId} jogador={jogador} />
        ))}
      </div>
    </div>
  );
...
function CardJogadorManual({
  ...
  function botaoEstaDesabilitado(indiceTime: number): boolean {
    ...
    if (indiceTimeAtribuido === indiceTime) return false;
...
export function SortearCampeonatoClient({
  tournamentId,
  squadSize,
}: {
  tournamentId: string;
  ...
  const numeroDeTimes = useMemo(() => {
    ...
    return Math.max(2, Math.floor(jogadoresDeCampo.length / squadSize));
  }, [jogadoresParaAtribuir, squadSize]);
  ...
  const overallsPorTime = useMemo(() => {
    ...
    return Array.from({ length: numeroDeTimes }, (_, indiceTime) => {
      const jogadoresDoCampoNoTime = jogadoresParaAtribuir.filter(
        (j) => !j.ehGoleiro && atribuicoes[j.registrationId] === indiceTime,
      );
      ...
      return Math.round((somaOverall / jogadoresDoCampoNoTime.length) * 10) / 10;
    });
  ...
  const totalJogadoresAtribuidos = useMemo(() => {
    ...
    return jogadoresParaAtribuir.filter(
      (j) =>
        atribuicoes[j.registrationId] !== undefined &&
        atribuicoes[j.registrationId] !== null,
    ).length;
  ...
  const contagemDeCampoPorTime = useMemo(() => {
    ...
    for (const jogador of jogadoresParaAtribuir) {
      ...
      const indice = atribuicoes[jogador.registrationId];
  ...
  const timesCheios = useMemo(() => {
    ...
    for (const [indice, quantidade] of contagemDeCampoPorTime) {
      if (quantidade >= squadSize) cheios.add(indice);
    }
    return cheios;
  }, [contagemDeCampoPorTime, squadSize]);
  ...
  async function buscarJogadores() {
    ...
    setErro(null);
    try {
      const resposta = await fetch(`/api/tournaments/${tournamentId}/draw`);
      if (!resposta.ok) {
        setErro("Erro ao carregar os jogadores inscritos.");
        return;
      }
      const dados: { jogadores: JogadorSorteado[] } = await resposta.json();
      setJogadoresParaAtribuir(dados.jogadores);
      setAtribuicoes({});
    } catch {
  ...
  function ativarModoManual() {
    ...
    setAtribuicoes({});
    if (!jogadoresParaAtribuir) buscarJogadores();
  }
  ...
  async function confirmarTimesManual() {
    ...
    try {
      const atribuicoesValidas = Object.entries(atribuicoes)
        ...
        .map(([registrationId, teamIndex]) => ({ registrationId, teamIndex }));

      const resposta = await fetch(`/api/tournaments/${tournamentId}/draw`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ manual_assignments: atribuicoesValidas }),
      });
  ...
  async function iniciarCampeonato() {
    ...
    setErro(null);
    try {
      const resposta = await fetch(`/api/tournaments/${tournamentId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      if (!resposta.ok) {
        const dados = await resposta.json();
        setErro(dados.error ?? "Erro ao iniciar o campeonato. Tente novamente.");
        return;
      }
      router.push(`/campeonatos/${tournamentId}`);
    } catch {
      setErro("Falha na conexão. Tente novamente.");
    } finally {
      setIniciando(false);
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      {/* Link de volta para o campeonato */}
      <div>
        <Link
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            ...
            <span className="font-medium text-foreground">
              {squadSize} de linha + 1 goleiro por time
            </span>{" "}
            conforme as configurações do campeonato.
          </p>
        </div>
      ...
      {/* Modo manual — atribuição individual dos jogadores */}
        ...
        <div className="space-y-4">
          {carregandoJogadores ? (
            ...
            <div className="bg-card rounded-xl ring-1 ring-foreground/10 p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Nenhum jogador inscrito no campeonato.
              </p>
            </div>
          ) : jogadoresParaAtribuir ? (
            <>
                  ...
                  >
                    ...
                    <div className="divide-y divide-border">
                      {jogadoresDoPosicao.map((jogador) => (
                        <CardJogadorManual
                          key={jogador.registrationId}
                          jogador={jogador}
                          numeroDeTimes={numeroDeTimes}
                          indiceTimeAtribuido={
                            atribuicoes[jogador.registrationId] ?? null
                          }
                          timesCheios={timesCheios}
                          aoAtribuir={(indice) =>
                            atribuirJogadorAoTime(jogador.registrationId, indice)
                          }
                        />
                      ))}
                    </div>
      ...
      {/* Resultado do sorteio ou atribuição manual confirmada */}
        ...
        <div className="space-y-4">
          ...
          {timesResultado.length === 0 ? (
            <div className="bg-card rounded-xl ring-1 ring-foreground/10 p-8 text-center">
              <Users className="size-8 text-muted-foreground/40 mx-auto mb-2" />
              <p className="text-sm text-muted-foreground">
                Nenhum time foi formado. Verifique se há jogadores inscritos no
                campeonato.
              </p>
            </div>
          ) : (
